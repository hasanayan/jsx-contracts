...
import type { PartName } from "./bound-names.js";
...
describe("arity guards", () => {
  ...
  it("rejects an exclusiveProps() first group with no props", () => {
    expect(() =>
      contract("Widget", "g").exclusiveProps([] as never, ["onClick"]),
    ).toThrow(
      new Error(
        'contract: component "Widget" calls exclusiveProps() with an empty ' +
          "group.",
...
describe("nameless contracts", () => {
  ...
  it("flattens a conjunction rather than nesting all inside all", () => {
    const built = contract("Widget", "g").when(
      ...
      contract().when(
        allOf(prop("tight").isPresent(), prop("flush").isPresent()),
        contract().forbidDescendants("Widget.Spacer"),
      ),
    );

    // Both arms flatten: neither the outer nor the inner conjunction survives
    // as an `all` of its own.
    expect(rowsFor(built, "subtree")[0]?.when).toEqual({
      all: [
        { prop: "variant", values: ["compact"] },
        { prop: "dense" },
