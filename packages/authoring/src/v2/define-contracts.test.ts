// Seam 1: authored input → emitted v2 rows. Registration, freeze, duplicate
// throws, second-map throws, dotted keys, `true`, closed-by-default, loose.

import { describe, expect, it } from "vitest";

import type { SlotsRowV2 } from "@jsx-contracts/eslint-plugin";
...
function onlyRow(rows: readonly unknown[]): SlotsRowV2 {
  expect(rows).toHaveLength(1);

  return rows[0] as SlotsRowV2;
}
...
describe("defineContracts", () => {
  ...
  it("emits the flat-config entry for the v2 children rule", () => {
    const set = defineContracts(({ contract }) => {
      contract("Card.Heading", CARD_FROM).slots({ ".Text": true });
    });
