// Seam 1: the descendants map, subtree bans, notInside and deprecated compile
// to subtree/ancestor rows. Dotted shorthands expand uniformly across every
// facet that accepts names — the regression the old emit path's silent
// `.Actions` no-op left open — and forbidDescendants takes all four entry forms.

import { describe, expect, it } from "vitest";

import type { ContractRowV2, FacetV2 } from "@jsx-contracts/eslint-plugin";
...
function rowOf<F extends FacetV2>(
  build: Parameters<typeof defineContracts>[0],
  facet: F,
): Extract<ContractRowV2, { facet: F }> {
  ...
  return row as Extract<ContractRowV2, { facet: F }>;
}
