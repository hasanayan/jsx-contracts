// Seam 2 (core): slots rows → bounds/requires/excludes verdicts over a
// synthetic tree, all branch-aware.
...
function slot(alias: string, extra: Partial<Slot> = {}): Slot {
  return {
    alias,
...
function row(slots: Slot[]): SlotsRow {
  return {
    facet: "slots",
