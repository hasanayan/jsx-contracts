...
// runtime validator — plus the authoring emitter. Nothing cross-checked them, so
// drift was silent. This is the cross-check: one curated, type-bound corpus run
// through both gatekeepers.
//
// The corpus is typed against rows.ts, so a field added or retyped there
...
import { contractRowsSchema } from "./rows-schema.js";
import type {
  AncestorRow,
  ContractRow,
  ContractRows,
