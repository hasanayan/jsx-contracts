 ...
 * rather than by container, it reports `misplaced` on a declared slot rendered
 * outside its container — the one slots check that fires on the slot element, so
 * it sits beside the generic per-container index rather than inside it.
 */
