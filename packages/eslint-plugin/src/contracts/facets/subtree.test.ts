...
import type { CombinedSubtree } from "./subtree.js";
import {
  combineSubtree,
  evaluateSubtree,
  prepareSubtreeRow,
...
// bounds must stay satisfiable: a descendant is never asked for more
// occurrences than it is allowed.
describe("combineSubtree crossed count bounds", () => {
  it("clamps a require entry's minCount to the maxCount another row tightened it past", () => {
    const combined = combineSubtree("Tabs.Root", [
      prepareSubtreeRow({
        facet: "subtree",
        importPath: "*/tabs",
        component: "Tabs.Root",
      ...
      prepareSubtreeRow({
        ...
        importPath: "*/tabs",
        component: "Tabs.Root",
        require: [{ name: "Tabs.List", max: 1 }],
      }),
    ]);

    expect(combined.require).toEqual([
      { name: "Tabs.List", minCount: 1, maxCount: 1 },
    ]);
  });
});
