// The count-default rule is implemented once per package on purpose (helpers
...
// bounds.
//
//   - `resolveCountBounds` (helpers, used by `findUnsatisfiable`'s crossed-bounds
//     verdicts) mirrors `resolveBounds` (core), which both facets call and which
//     is therefore the canonical statement of the rule. The field names differ
...
import { resolveBounds } from "../../eslint-plugin/build/contracts/model.js";

import { resolveCountBounds } from "./unsatisfiable.js";
...
describe("the count-default rule agrees across the two packages", () => {
  describe("resolveCountBounds tracks resolveBounds", () => {
    for (const { label, minCount, maxCount } of declarations) {
      it(label, () => {
        const helpers = resolveCountBounds(minCount, maxCount);
        const core = resolveBounds(minCount, maxCount);

        expect(helpers.min).toBe(core.minCount);
        expect(helpers.max).toBe(core.maxCount);
      });
    }
  });
